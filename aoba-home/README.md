...
* [`src/clj/aoba_home/`](src/clj/aoba_home/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/aoba_home/`](src/cljs/aoba_home/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/aoba_home/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `aoba-home` project root directory
...
in the [`src/clj/aoba_home/`](src/clj/aoba_home/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/aoba_home/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns aoba-home.example
  (:require [aoba-home.config :as config])
